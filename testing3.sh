...
# Program to execute
PROGRAMS=( \
"./bin/filtered_main_graph 5 80 1.2 2 ./datasets/smallscale/dummy-data.bin" \
"./bin/filtered_main_graph 10 80 1.2 2 ./datasets/smallscale/dummy-data.bin" \
)
RESULTS_FILE="results3.txt"
TEMP_FILE="temp_output3.txt"
RUNS=5
...
for ((p=0; p<${#PROGRAMS[@]}; p++)); do
    ...
    if [ $p -eq 0 ]; then
        ...
        echo "Program $((p+1)) was $improvement% faster than the base program with average time: $average_runtime" >> "$RESULTS_FILE"
    fi
done
