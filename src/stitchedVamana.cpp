...
Graph stitchedVamana(vector<vector<double>> &coords, set<int> F, double a, int L_small, int R_small, int R_stitched)
{
    ...
    for (int f : F)
    {
        ...
        {
            Graph Gf;
            G.graphUnion(Gf); // add the empty graph without calling vamana to avoid extra work
            continue;
        }
