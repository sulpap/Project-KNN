...
#include "../include/stitchedVamana_medoid_parallel_2.hpp"
...
unordered_map<int, set<int>> compute_Fx_medoid_parallel_2(vector<vector<double>> &coords) 
{
...
unordered_map<int, vector<Node *>> compute_PfMap_medoid_parallel_2(vector<vector<double>> &coords, set<int> F) 
{
...
void store_medoid_medoid_parallel_2(Graph &G, map<int, Node *> &medoids, int f, int medoidId)
{
...
Graph stitchedVamana_Medoid_Parallel_2(vector<vector<double>> &coords, set<int> F, double a, int L_small, int R_small, int R_stitched, map<int, Node *> &medoids) 
{
    // 1. Initialize G = (V, E) to an empty graph
    Graph G;

    // 2. Let Fx ⊆ F be the label-set for every x ∈ P
    unordered_map<int, set<int>> Fx = compute_Fx_medoid_parallel_2(coords); // Fx maps node IDs to their label sets

    // 3. Let Pf ⊆ P be the set of points with label f ∈ F
    unordered_map<int, vector<Node *>> PfMap = compute_PfMap_medoid_parallel_2(coords, F); // Map each label f to its corresponding set of points (coordinates)
    ...
    for (int f : F) 
    {
        ...
        int medoidId = Vamana_Medoid_Parallel_2(Gf, Pf, R_small, a, L_small); // we also give f, because vamana creates the graph, so it
        ...
        store_medoid_medoid_parallel_2(G, medoids, f, medoidId);
    }
