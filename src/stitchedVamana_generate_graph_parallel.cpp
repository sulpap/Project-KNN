...
#include "../include/stitchedVamana_generate_graph_parallel.hpp"
#include "../include/vamana_generate_graph_parallel.hpp"
...
Graph stitchedVamana_Generate_Graph_Parallel(vector<vector<double>> &coords, set<int> F, double a, int L_small, int R_small, int R_stitched, map<int, Node *> &medoids) 
{
    ...
    unordered_map<int, set<int>> Fx = compute_Fx_generate_graph_parallel(coords); // Fx maps node IDs to their label sets

    // 3. Let Pf ⊆ P be the set of points with label f ∈ F
    unordered_map<int, vector<Node *>> PfMap = compute_PfMap_generate_graph_parallel(coords, F); // Map each label f to its corresponding set of points (coordinates)
    ...
    for (int f : F) 
    {
        ...
        int medoidId = Vamana_Generate_Graph_Parallel(Gf, Pf, R_small, a, L_small); // we also give f, because vamana creates the graph, so it
        ...
        store_medoid_generate_graph_parallel(G, medoids, f, medoidId);
    }
